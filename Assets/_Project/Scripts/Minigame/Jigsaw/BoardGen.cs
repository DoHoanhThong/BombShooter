...
public class BoardGen : MonoBehaviour
{
    ...
    Sprite LoadBaseTexture()
    {
        ...
        tex = ResizeToNearestMultipleOf100(tex);
        if (!tex.isReadable)
        {
    ...
    Texture2D ResizeToNearestMultipleOf100(Texture2D inputTexture)
    {
        int newWidth = Mathf.CeilToInt(inputTexture.width / 100f) * 100;
        int newHeight = Mathf.CeilToInt(inputTexture.height / 100f) * 100;

        Texture2D resizedTexture = new Texture2D(newWidth, newHeight);
        Color[] originalPixels = inputTexture.GetPixels();
        Color[] resizedPixels = new Color[newWidth * newHeight];

        float widthRatio = (float)inputTexture.width / newWidth;
        float heightRatio = (float)inputTexture.height / newHeight;
        ...
        for (int y = 0; y < newHeight; y++)
        {
            for (int x = 0; x < newWidth; x++)
            {
                ...
                resizedPixels[y * newWidth + x] = originalPixels[originalY * inputTexture.width + originalX];
            }
        }
